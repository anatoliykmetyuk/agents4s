...
description: >
  ...
  skill: `# <Actor Name> Actor Specification`, shared message and term definitions
  under `specs/`, `## Receives` as name-only lists) into a
  Scala 3 Apache Pekko Typed project: messages.scala generated from the message
  catalog, one object per actor, union-typed AcceptedMessages, declarative behavior
  defs, and LLM steps via agents4s LLMActor + CursorAgent. Use this skill whenever
  the user wants to harness specs into runnable Scala, scaffold Pekko actors from
  actor-spec markdown, implement an agentic workflow as actors, or mentions
  "actor-harness", "harness specs", "specs to Scala", "LLMActor from specs", or
  wants to add a new actor from a freshly added specs/*.md that follows the
  actor-spec format. Also use when they describe workers, validators, orchestrators,
  or subagents spawned from specs—even if they do not say "Pekko" by name. Pair
  with the actor-spec skill for writing/editing specs.
---
...
**Input contract:** specs are written with the **actor-spec** skill. Canonical title: `# <Actor Name> Actor Specification`. Sections typically include **Actor Purpose**, optional **`## Suite references`** or inline **`## Definitions`** (per actor-spec), **`## Receives`** (**message names only**, backticks, one per line), **Workflow**. **Full** message signatures live in whatever markdown file holds the suite’s message definitions (see Step 1); the harness generates **`messages.scala`** from that file.

**Why this split:** actors give explicit message types, retries, and composition; **`LLMActor`** encapsulates the Cursor session, JSON result file, and uPickle parsing (see [references/library-api.md](references/library-api.md)). Shared protocol types live in **`messages.scala`**, sourced from the suite’s message-definitions markdown under **`specs/`** (see [references/actor-translation-guide.md](references/actor-translation-guide.md)).

## When this applies

- A **`specs/`** directory containing **actor specifications** (title pattern above) and, somewhere under **`specs/`**, the suite’s **message definitions** and (when present) **term definitions** (see Step 1).
...
my-harness/
├── specs/
│   └── <user-chosen>.md   # message definitions (full signatures) — locate under specs/
├── scripts/
│   ├── setup.sh
│   ├── run.sh
│   └── test.sh
├── src/main/scala/<pkg>/
│   ├── messages.scala     # generated from the suite message-definitions markdown
│   ├── Main.scala
